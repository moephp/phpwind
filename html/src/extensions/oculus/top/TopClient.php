...
class TopClient
{
	...
	private $top_key = "23731570";

	private $top_securt = "e95a40d05279d1b2b498115848e43fa9";
	...
	protected function generateSign($params)
	{
		...
		$stringToBeSigned = $this->top_securt;
	...
	protected function logCommunicationError($apiName, $requestUrl, $errorCode, $responseTxt)
	{
		...
		$logger->conf["log_file"] = rtrim(TOP_SDK_WORK_DIR, '\\/') . '/' . "logs/top_comm_err_" . $this->top_key . "_" . date("Y-m-d") . ".log";
	...
	{
        Wind::import('EXT:oculus.top.ResultSet');
		...
		$sysParams["app_key"] = $this->top_key;
		...
		{
            Wind::import('EXT:oculus.top.TopLogger');
			$logger = new TopLogger;
			$logger->conf["log_file"] = rtrim(TOP_SDK_WORK_DIR, '\\/') . '/' . "logs/top_biz_err_" . $this->top_key . "_" . date("Y-m-d") . ".log";
			$logger->log(array(
				date("Y-m-d H:i:s"),
				$resp
			));
